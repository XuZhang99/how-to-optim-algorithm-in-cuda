...
本文记录下最近使用 KDA（https://github.com/mit-han-lab/kernel-design-agents） 优化SGLang Diffusion kernel 的流程。主要内容包括流程中暴露出的工程问题和解决方法、prompt 约束和验证规则。

对比了我之前在kernel-pliot里面的 [humanize loop的方式](https://mp.weixin.qq.com/s/pScZ_9cA-6cWUPjfcGjNyg) 以及部分公开的其它Kernel Agent框架，评测下来感觉还是 KDA 的流程灵活性和上限的平衡比较好。然后最近几天把 https://github.com/BBuf/kernel-pilot 完全切换到了 KDA 风格的任务组织方式，用于批量生成和验证 SGLang Diffusion Kernel 优化任务。
...
## 0x1.3 Benchmark 和.cu的导出方式需要固定成模板

早期流程尝试通过 `kda_kernels` overlay 将优化后的 kernel 接入 SGLang。该方式在 KernelPilot #25（https://github.com/BBuf/kernel-pilot/pull/25）暴露出一个问题：overlay promotion 会用 `setattr` 将 SGLang 中由 `@register_custom_op` 修饰的 public op 替换为普通 Python dispatcher。此时测得的“加速”可能来自绕过 host wrapper、custom op registration 或 torch.compile 相关逻辑，而不是 device kernel 本身变快。

SGLang 的生产路径需要保留 public entry point 语义，例如 `@register_custom_op` 对 `torch.compile` 和 CUDA graph 兼容性有影响。移除这一层后得到的 benchmark 数字不能直接代表生产路径性能。

kernel-pilot 后续采用 standalone diffusion benchmark contract并提供相同的benchmark和.cu导出python接口模板：

- benchmark runtime 不 patch、不 import、不 monkey-patch SGLang 源码。
- Agent 从最新 upstream SGLang `main` 复制相关 kernel 源码到当前任务的 `baseline/`。
- `baseline/` 和 `solution/` 通过同一套本地 ABI 暴露；如果是 `.cu`，两边都使用相同的 tvm-ffi CUDA wrapper，wrapper overhead 对称。
- 输出 tensor 都采用 destination-passing，避免一边在 timed path 中分配，一边不分配。
- workload 在 tuning 前冻结，后续不修改 shape、tolerance 或 scoring。
- `bench/benchmark.py` 从统一模板开始，包含 isolated subprocess、fresh inputs、poison outputs（先写入哨兵值，检查输出是否真的被覆盖）、correctness-before-timing、CUDA event、inner-loop amplification（单次计时中重复调用以放大计时信号）、interleaved A/B、per-row median/mean/std/min/p10/p90。这里参考了一下 AKO4X的benchmark模板，具体见： https://github.com/TongmingLAIC/AKO4X/tree/main/templates/skills/benchmark
...
- 用 wall-clock 观察 Python wrapper 变化，却将结果归因于 device kernel。
...
如果 prompt 未明确允许 shape-specialized dispatch，Agent 可能生成一个覆盖所有 shape 的通用 kernel。该策略会导致部分 shape 加速、部分 shape 回归，geomean 仍可能保持可接受，但生产 workload 可能命中回归 bucket。

kernel-pilot 的 `diffusion_kernel_rules.md` 已明确允许 shape-specialized kernels、template variants、autotune tables 和 dispatchers。使用 dispatch 时需要在 `docs/dispatch.md` 记录：
...
因此 prompt 中需要明确：任务可根据 shape、layout、dtype 或平台差异使用不同 kernel/config，并通过 fail-closed dispatcher 保持生产路径可解释；不满足条件时回退到 baseline。
...
- no-go 需要 correctness、benchmark/NCU evidence 和明确 active bound，不能只基于第一轮回归判断。
...
KernelPilot 将 `KernelWiki` 和 `ncu-report-skill` 放在 `external/` 下作为外部知识源。diffusion kernel 规则要求每轮 RLCR 在决定下一次 code edit、benchmark、profile 或 no-go 之前，刷新 task prompt、当前 benchmark/profile evidence、`KernelWiki` 和 `ncu-report-skill`。因此，这两类知识源属于每轮迭代上下文，而不是一次性的 preflight 材料。
...
实验中，KDA 在一天内完成了一批 SGLang Diffusion kernel 的 microbench 优化，包括 B200 `fuse_scale_shift` 的 `2.7570x` geomean，B200 `group_norm_silu` 的 `2.2794x` geomean，H200 `norm_scale_shift` 的 `1.27x` geomean。该流程同时记录了 benchmark 对称性、baseline provenance、correctness 和 review hardening。优化后的kernel可以被我们按需取用，用到真实的模型推理框架中。
