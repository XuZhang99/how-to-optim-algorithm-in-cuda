...
图 2：KDA-Pilot 在 B200 上跟踪了 10 个 SGLang diffusion kernel task。数值行给出 wall-geomean speedup；这里的 wall time 包含 Python dispatch、wrapper、kernel launch 和 `cuda.synchronize()` 能观察到的同步开销，比单纯 kernel device time 更接近真实调用路径。`review` 行保留的是已经转成 SGLang fast path、但原始 task baseline 后来发生变化的任务。
...
| `residual_gate_add`         | `review`          | one-pass CUDA fusion、pinned-GPU correctness、baseline swap 后重新标注 benchmark 口径 |
