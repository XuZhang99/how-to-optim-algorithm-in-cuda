...
- 定位上，`/goal` 是运行时底座，Humanize 1.0 是方法论；`/goal` 现在还缺跨模型的独立审查和可复盘的外部记录，这两块仍要靠 Humanize 1.0 补。

# 0x1. 前言

Humanize 1.0 的核心是 RLCR（Ralph-Loop with Codex Review），本质是一套外挂的 bash 状态机：用 Stop hook 拦住退出，把状态落到 `.humanize/` 的 markdown 里，再另起一个独立的 Codex 进程做审查，不通过就继续下一轮。它解决的问题很明确：让"完成"变成一个外部判定，而不是 Agent 自己说完就完。

后来 Codex 自己上了 `/goal`，文档把它定义成"长时间工作的持久目标"。当时我的第一反应是：这不就是把 Humanize 1.0 干的事内置了一份吗？所以这次我直接把 `openai/codex` 拉下来，把 `/goal` 的实现从头读了一遍（commit `04483f4`，goal 相关代码集中在 `codex-rs/ext/goal/`）。读完之后结论反过来了：`/goal` 和 Humanize 1.0 根本不在一个层面上，同一组职责，两边放的位置几乎是反的。这篇就把代码摊开，讲清楚 `/goal` 怎么实现、亮点在哪，以及它和 Humanize 1.0 的区别到底在哪。
...
读 `/goal` 第一个反直觉的点：我本来以为它就是 TUI 里某个 slash command 的处理函数，结果它是一个挂在 Agent 事件循环上的 extension。`codex-rs/ext/goal/src/extension.rs` 里的 `GoalExtension` 实现了一整排 trait：
...
cookbook 里只讲了 active/paused/complete/budget-limited，实际代码里多了 `Blocked` 和 `UsageLimited`，这两个恰恰是长任务里最容易出事的地方（模型钻死胡同、账号额度耗尽），后面会看到它们都有专门的兜底逻辑。`is_terminal()` 只认 `BudgetLimited | Complete`，也就是说 `Blocked`、`Paused`、`UsageLimited` 都不是终点，都还能被恢复。

另一个是 token 和时间，它俩是表里的一等字段：`tokens_used`、`time_used_seconds`、`token_budget` 直接进 schema，不是事后从日志里捞。这就让预算判定可以做成一条原子 SQL（见 0x6），而不是 Humanize 1.0 那种"数到第几轮了"的近似。Humanize 1.0 的"预算"是 `--max N`，默认 42 轮，本质是数迭代次数；`/goal` 这边记的是真实 token 数和真实耗时（wall-clock），其中只有 token 能设硬上限（怎么设、怎么影响停止，见 0x6）。一个是"你还能折腾几次"，一个是"你还能烧多少钱"，后者对长任务明显更贴近实际成本。
...
也就是说，只有当前没有 turn 在跑、用户也没有排队输入时，才会触发一次"要不要继续"的判断。这正是 cookbook 说的"只有线程空闲、Goal 活跃且预算充足时才会继续"，落到代码就是这两个 if。
...
它必须在每一轮开头让模型重新读 plan、读 goal-tracker、读上一轮 summary，因为每一轮都是新进程，上下文是从文件里重建的。Re-anchor、round contract、task lane（mainline/blocking/queued 三种 tag）、bitlesson 选择……这一大套脚手架，本质都是在"重新把记忆灌回模型脑子里"。
...
命中 prompt cache 的 input token 不计入预算，只算未命中的 input 加 output。长任务里上下文越滚越大，如果把缓存的 input 也算进去，预算会被"重复读同一段上下文"飞快吃光，所以这个减法对长任务很关键。
...
注入的 `budget_limit.md` 内容是："系统已经把 goal 标成 budget_limited，别开新活了，赶紧收尾：总结进度、列出剩余工作和阻塞、给用户一个明确的下一步。" 它特意强调撞预算不等于完成：`Do not call update_goal unless the goal is actually complete.` 这正是 cookbook 说的"达到预算上限不等于完成目标"，模型不能因为没钱了就把活儿标成 done。

Humanize 1.0 这边对应的是"跑满 42 轮就停"，相对粗一些，而且它没有"算 token"的概念，成本控制的单位是"轮"，不是"钱"。

# 0x7. continuation.md：行为约束与完成判定

读到这我才意识到，前面那些 Rust 代码都是骨架，`/goal` 的行为几乎全压在 `prompts/templates/goals/continuation.md` 这 50 行 prompt 里。它只干两件事，但都正对着 LLM 自主长跑的两个头号失败模式。
...
翻译成人话：不许为了"让测试变绿"去换一个更小更安全的解法。这是把目标漂移当成头号敌人在防。

另一个是过早宣布完成，这也正是 Humanize 1.0 整套 RLCR 存在的理由。`/goal` 拿一段非常严格的"完成审计"来对付：
...
最后那句 `The audit must prove completion, not merely fail to find obvious remaining work.` 是整段的精华：没发现明显的剩余工作，不等于证明了已经完成，举证责任压在"完成"这一侧。逐条要求都有现状证据支撑，才能调 `update_goal(complete)`。

还有一个我没想到的设计，blocked 的迟滞阈值：
...
`/goal` 和 Humanize 1.0 在"完成判定"上的根本分歧，到这里就摆出来了。

Humanize 1.0 是外部对抗式审查。这里得先把"吐 `COMPLETE`"讲清楚，不然容易当成一句口号。Humanize 1.0 的 RLCR 是这么转的：干活的实现者（在我的用法里是 Claude）跑完一轮，把这一轮做了什么写进 `round-N-summary.md`，然后正常退出；退出的瞬间 Stop hook 触发，另起一个独立的 Codex 进程（`codex exec`，默认 `gpt-5.5:high`），把原始 plan、Claude 这轮的 summary、commit 历史、goal-tracker 一起塞给它，让它当审查员。这个审查 prompt（`regular-review.md`）相当严格：要求它 "meticulous and skeptical"、"DO NOT follow its lead"（不许跟着实现者一起放水）、deferred 的任务一律按未完成处理，只有当所有 AC 都满足、没有任何遗留时，才允许在审查结果的最后一行单独输出一个词 `COMPLETE`；否则就把发现的问题写进 `round-N-review-result.md`。

关键在于，这个 `COMPLETE` 不是写给人看的，是写给 bash 看的。Stop hook 会去 grep 审查结果的最后一行：grep 不到 `COMPLETE`，就把这份 review 当成下一轮的 prompt 喂回给实现者，循环继续；grep 到了，才放行进入第二阶段 `codex review --base <branch>`，让 Codex 真正去审代码 diff，按 `[P0-9]` 标严重度，有问题接着改，没问题才收尾。所以 `COMPLETE` 本质就是一个机器可读的放行信号：判"完成"的那个模型实例跟干活的不是同一个，实现者无权自己宣布完成。
...
要说哪种更难被糊弄，是 Humanize 1.0：两个模型相互独立，实现者再自信也得过审查这关。`/goal` 的自审省掉一个进程、省掉一堆 token，但天花板取决于模型自己的诚实度。同一个模型既当运动员又当裁判，理论上是能自我说服的。它用那段极其克制、反复强调 "unproven / must prove / not achieved" 的 prompt 去顶高这个天花板，但这和"换一个独立模型来 review"终究不是一回事。

# 0x8. 定位辨析：/goal 与 Humanize 1.0 的关系

读完我的判断是，`/goal` 和 Humanize 1.0 不在一个层面上，是上下层的关系。

`/goal` 回答的是运行时层面的问题：*Agent 怎么在没人催的情况下自己接着干、并且基于证据判断自己干完没有？* 它把持久目标状态、空闲续跑、预算记账、完成/阻塞的自审做成了 Codex 的原生能力，而且做得很扎实：事务化状态、race-free 记账、安全边界调度、权限分离、出错熔断。

Humanize 1.0 回答的是方法论层面的问题：*我怎么让"完成"这件事可信、并且让一场十几小时的搜索不要把试过的东西忘掉？* 它有两样东西是 `/goal` 现在没有的：

1. 跨模型的独立对抗审查。`/goal` 是自审，Humanize 1.0 是请第二个模型来挑刺，在"防止过早完成"这个点上，外部裁判结构上更硬。
2. 可检索、可复盘的外部记录。Humanize 1.0 每一轮都把状态落到文件：`goal-tracker.md` 里有不可变的目标加 AC、可变的 Active/Completed/Deferred 任务，还有一份 Plan Evolution Log（任何计划变更都要写理由）；外加每轮的 `round-N-summary.md` / `round-N-review-result.md` 和 BitLessons。这些是 git diff 得动、人能直接翻的外部记忆，也是它给跨模型审查喂的"案卷"。`/goal` 的状态里只有 objective、status、预算，没有"这一轮干了什么、审查发现了什么、为什么 defer 了某个任务"这种跨轮的结构化记录。线程上下文虽然连续，但它会被压缩、会滚出窗口，不是为"几十轮可复盘"设计的账本。

所以更准确的说法是，`/goal` 是底座，Humanize 1.0 是跑在底座上的方法论。这两层理论上能拼起来：让 `/goal` 接管"空闲了自动接着跑、token 烧到预算就停、出错就熔断"这些运行时脏活，Humanize 1.0 那层只保留它真正不可替代的东西，跨模型的审查门禁和那套可复盘的外部记录。

`/goal` 把"怎么持续推进"做成了引擎，Humanize 1.0 把"怎么相信它真的完成了、怎么不让它失忆"做成了方法。`/goal` 是 OpenAI 在 Codex 里内置的运行时能力，Humanize 1.0 是套在 Agent 外面的一层工程纪律。

# 0x9. 值得借鉴的工程细节

抛开宏观对比，`/goal` 代码里有几处我打算直接借鉴到自己 harness 里的细节：
...
`/goal` 这次让我有点意外的不是它"内置了 Humanize 1.0"，而是它有多克制：状态就一行 SQLite，prompt 就 50 行，工具就三个、还砍到模型只能 complete/blocked。它没去抢 Humanize 1.0 那层方法论的活，没有独立的跨模型审查，也没有可复盘的外部记录。它只把运行时这一层做到位：事务化的持久目标、安全边界的空闲续跑、诚实的 token 记账、出错熔断、权限分离。

对我做 SGLang / kernel 长期优化的实际影响是：续跑这块的引擎可以放心交给 `/goal`，不用再自己用 bash hook 去模拟"线程空闲了要不要继续"；而真正决定长任务能跑多深的那两件事，独立的对抗审查和可复盘的外部记录，`/goal` 暂时还不管，得继续靠 Humanize 1.0 这层方法论补上。两者叠起来，才是 AVO 那种 autonomous search 想要的完整形态。
